...
def _update_solver_executable(alias: str, executable: str) -> dict:
    current = settings()
    existing_config = {}
...
async def _verify_solver_install(alias: str, executable: str | None = None) -> dict:
    current = settings()
    spec = current.solver_install_specs.get(alias)
    if spec is None:
        raise HTTPException(status_code=404, detail=f"Solver install spec not found: {alias}")
    ...
    exit_code, stdout_bytes, stderr_bytes = await _run_local_command(command, workdir, timeout=20)
...
def configure_solver_executable(alias: str, payload: T_SolverExecutable = Body(...)):
    ...
    return {
        ...
        "data": _update_solver_executable(alias, executable),
    }
...
async def verify_solver_installation(alias: str, payload: T_SolverExecutable = Body(default={})):
    return {
        ...
        "data": await _verify_solver_install(alias, payload.get("executable")),
    }
